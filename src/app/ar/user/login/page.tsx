...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (loginError) {
      ...
      // استخدام router.push للانتقال الآمن والسريع إلى لوحة التحكم
          ...
          <div className="flex flex-col lg:flex-row gap-16 items-center relative z-10">
            <div className={`flex-1 space-y-8 ${isRtl ? 'text-right' : 'text-left'}`}>
              <div className={`flex items-center gap-3 ${isRtl ? 'justify-start' : 'justify-start'}`}>
                ...
                <h2 className="text-xs font-black tracking-[0.3em] text-zinc-600 uppercase">المعرّف الآمن Safi</h2>
              </div>
              
              <h1 className="text-5xl font-black tracking-tighter leading-[0.9] text-white uppercase italic">
                وصول <br/> <span className="text-amber-500 text-6xl">عالمي</span>
              </h1>
              
              <p className="text-zinc-500 text-sm font-medium leading-relaxed">
                افتح بوابتك الخاصة إلى SafiPay. الأمان هو أولويتنا الدائمة.
              </p>
            </div>
            ...
            <div className="w-full lg:w-[320px] space-y-4">
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                >
                  <span className="uppercase tracking-widest text-xs">{loading ? 'جارٍ التحقق...' : 'الدخول إلى البوابة'}</span>
                  {isRtl ? (
              ...
              <div className="text-center pt-2">
                <Link href={`/${lang}/user/signup`} className="group flex items-center justify-center gap-2">
                  <span className="text-zinc-600 text-[10px] font-bold uppercase tracking-widest">هل أنت جديد هنا؟</span>
                  <span className="text-amber-500 text-[10px] font-black uppercase tracking-widest group-hover:border-b border-amber-500 transition-all">الانضمام إلى قائمة الانتظار</span>
                </Link>
              </div>
            </div>
          </div>
