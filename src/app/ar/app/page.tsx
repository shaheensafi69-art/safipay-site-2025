      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        <div className="flex flex-col md:grid md:grid-cols-2 gap-12 md:gap-16 items-center">
          ...
          >
            ...
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-black mb-6 tracking-tighter italic leading-[1.1]">
              مستقبل <span className="text-amber-500 text-glow">المال</span> <br className="hidden md:block" /> 
              في جيبك
            </h1>
            <p className="text-gray-400 text-lg md:text-xl mb-10 leading-relaxed max-w-xl mx-auto md:mx-0 font-medium text-justify">
              نحن لا نبني مجرد تطبيق؛ بل نفتح بوابة نحو الحرية المالية. سريع، آمن، وبدون حدود.
            </p>
            <div className="flex flex-wrap justify-center md:justify-start gap-5">
              <StoreButton img="https://upload.wikimedia.org/wikipedia/commons/7/78/Google_Play_Store_badge_EN.svg" text="قريباً في متجر بلاي" />
              <StoreButton img="https://upload.wikimedia.org/wikipedia/commons/3/3c/Download_on_the_App_Store_Badge.svg" text="قريباً في متجر تطبيقات أبل" />
            </div>
          </motion.div>
                    ...
                    <AnimatePresence mode="wait">
                      {appState === 'loading' && (
                        <motion.div key="l" exit={{ opacity: 0 }} className="absolute inset-0 flex flex-col items-center justify-center bg-black">
                           ...
                           <span className="text-amber-500 font-bold text-[10px] tracking-[0.5em]">جاري التشفير</span>
                        </motion.div>
                      )}
                      ...
                      {appState === 'dashboard' && (
                        <motion.div key="d" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 bg-[#080808] flex flex-col pt-12 text-left" dir="ltr">
                          ...
                          <div className="px-6 mb-8">
                            <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest mb-1">إجمالي الرصيد</p>
                          ...
                          <div className="mt-auto bg-[#111] rounded-t-[3rem] p-7 border-t border-white/10">
                            <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-6">النشاط الأخير</h4>
                            <div className="space-y-5">
                              <TransactionItem type="up" title="تحويل وارد" amount="+ 15k AFN" />
                              <TransactionItem type="down" title="رسوم الصرف" amount="- $12.00" />
                            </div>
                          </div>
                        </motion.div>
                      )}
        ...
        <div className="mt-32 md:mt-48 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 text-right">
          <FeatureCard 
            icon={<Zap size={28} />} 
            title="سرعة خاطفة" 
            desc="نظام تبادل حديث مصمم لتقليل وقت المعاملات على مستوى العالم." 
            delay={0.1} 
          />
          <FeatureCard 
            icon={<ShieldCheck size={28} />} 
            title="أمان بارد" 
            desc="أعلى مستوى من التشفير والتخزين البارد لحماية أصولك الرقمية." 
            delay={0.2} 
          />
          <FeatureCard 
            icon={<Smartphone size={28} />} 
            title="واجهة ذكية" 
            desc="مصممة لتبسيط إدارة التداولات المالية الأكثر تعقيداً." 
            delay={0.3} 
          />
          <FeatureCard 
            icon={<Clock size={28} />} 
            title="دعم ٢٤/٧" 
            desc="فريق الدعم الفني لدينا متاح دائماً في أي وقت تحتاج فيه للمساعدة." 
            delay={0.4} 
          />
        </div>
        ...
        >
          <h2 className="text-3xl md:text-5xl font-black mb-12 italic uppercase tracking-tighter text-amber-500">ابدأ الآن</h2>
          <div className="grid md:grid-cols-3 gap-12">
            <Step num="١" title="تحميل" desc="انضم إلى قائمة الانتظار لتكون من أوائل الذين يخوضون تجربة التطبيق." delay={0.1} />
            <Step num="٢" title="توثيق" desc="أكمل عملية KYC الآمنة لتفعيل محفظتك العالمية." delay={0.2} />
            <Step num="٣" title="تداول" desc="ابدأ في تبادل وإدارة أصولك المالية دون قيود أو حدود." delay={0.3} />
          </div>
        </motion.div>
      </div>
    </div>
...
function TransactionItem({ type, title, amount }: any) {
  return (
    <div className="flex justify-between items-center" dir="rtl">
      ...
      <div className={`text-[11px] font-black dir-ltr ${type === 'up' ? 'text-green-500' : 'text-white'}`}>{amount}</div>
    </div>
  );
}
...
function FeatureCard({ icon, title, desc, delay }: any) {
  return (
    ...
    >
      <div className="mb-6 p-4 bg-amber-500/10 text-amber-500 w-fit rounded-2xl mr-0 group-hover:bg-amber-500 group-hover:text-black transition-colors">{icon}</div>
      <h3 className="text-xl font-black uppercase italic tracking-tighter mb-3">{title}</h3>
      <p className="text-gray-400 text-sm leading-relaxed text-justify">{desc}</p>
    </motion.div>
  );
}
