...
export default function InvestPageRU() {
  const CAMPAIGN_LINK = "https://gogetfunding.com/safipay-%d8%a7%d9%88%d9%84%db%8c%d9%86-%d8%a8%d8%a7%d9%86%da%a9-%d8%af%db%8c%d8%ac%db%8c%d8%8c-%d8%a8%d8%b1%d8%a7%db%8c-%d9%85%db%8c%d9%84%db%8c%d9%88%d9%86%d9%87%d8%a7-%d8%a7/";
  const raised = 0;
  const goal = 80000;
    ...
    <div className="min-h-screen bg-gray-950 text-white font-sans selection:bg-amber-500/30" dir="ltr">
      ...
      <section className="relative pt-32 pb-20 overflow-hidden border-b border-amber-900/20">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          ...
          >
            <Target size={16} />
            <span>Официальная кампания по сбору средств и поддержке</span>
          </motion.div>
          ...
          >
            Построим финансовое будущее <br /> Афганистана вместе
          </motion.h1>
          ...
          >
            SafiPay создает первый глобальный необанк для афганцев. При вашей поддержке мы получим необходимые лицензии, разовьем технологии безопасности и устраним финансовый разрыв для миллионов людей.
          </motion.p>
        </div>
      </section>
          ...
          >
            <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-8">
              <div className="text-left">
                <span className="text-gray-500 uppercase tracking-widest font-bold text-sm">Цель кампании</span>
              ...
              <div className="text-right">
                ...
                <span className="text-amber-500/60 uppercase tracking-widest font-bold text-sm">Собрано</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12 border-t border-white/5 pt-12">
              <div className="text-center">
                ...
                <div className="text-sm text-gray-500 font-bold tracking-tighter">Собрано средств</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-white mb-1">{donors.toLocaleString()}</div>
                <div className="text-sm text-gray-500 font-bold tracking-tighter">Участников</div>
              </div>
              <div className="text-center">
                <div className="text-sm text-amber-500/80 font-mono">Последнее обновление: {new Date().toLocaleDateString('ru-RU')}</div>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <section className="py-24 bg-black/30 border-y border-white/5">
        <div className="container mx-auto px-6 text-center">
          <h2 className="text-4xl font-black mb-16">Выберите ваш вклад в будущее</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
            {[
              { amount: 10, label: "Друг проекта", desc: "Маленький вклад — большое сердце" },
              { amount: 25, label: "Партнер роста", desc: "Укрепление нашей инфраструктуры" },
              { amount: 100, label: "Посол перемен", desc: "Ключевая роль в успехе проекта" },
              { amount: "Любая", label: "Стратег", desc: "Создайте свое наследие", custom: true }
            ].map((item, idx) => (
              <motion.a
                key={idx}
      ...
      <section className="py-24 container mx-auto px-6">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div className="space-y-8 text-left">
            <h2 className="text-4xl font-bold">Полная прозрачность работы</h2>
            <div className="space-y-6">
              {[
                { icon: <ShieldCheck className="text-amber-500" />, title: "Безопасные транзакции", text: "Все средства проходят через международную платформу GoGetFunding с полной защитой." },
                { icon: <TrendingUp className="text-amber-500" />, title: "Стратегическое распределение", text: "Инвестиции идут напрямую на получение лицензий и развитие технических систем." },
                { icon: <Users className="text-amber-500" />, title: "Ежемесячные отчеты", text: "Все сторонники получают детальные отчеты о расходовании привлеченных средств." }
              ].map((feature, i) => (
          ...
          <div className="bg-gradient-to-br from-amber-600 to-amber-900 p-12 rounded-[3rem] text-center shadow-2xl shadow-amber-900/20">
            <h3 className="text-3xl font-black mb-6 italic text-white leading-relaxed">"Инвестируйте в нацию, <br /> а не просто в банк."</h3>
            <p className="text-white/80 mb-8 font-light">Ваш вклад приближает финансовую независимость миллионов афганцев.</p>
            <motion.a 
              href="/ru/contact"
