  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#050505] text-white pt-20 md:pt-32 pb-10 px-4 md:px-6 relative overflow-hidden" dir="ltr">
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        <div className="flex flex-col md:grid md:grid-cols-2 gap-12 md:gap-16 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/5 border border-white/10 rounded-full text-amber-500 text-[10px] md:text-xs font-black tracking-widest mb-6 uppercase">
              ...
              Экосистема SafiPay в эфире
            </div>
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-black mb-6 tracking-tighter italic leading-[0.9]">
              Будущее <span className="text-amber-500 text-glow">финансов</span> <br className="hidden md:block" /> 
              в вашем кармане
            </h1>
            <p className="text-gray-400 text-lg md:text-xl mb-10 leading-relaxed max-w-xl mx-auto md:mx-0 font-medium">
              Мы создаем не просто приложение, а открываем дверь к финансовой свободе. Быстро, безопасно и без границ.
            </p>
            <div className="flex flex-wrap justify-center md:justify-start gap-5">
              <StoreButton img="https://upload.wikimedia.org/wikipedia/commons/7/78/Google_Play_Store_badge_EN.svg" text="Скоро в Play Store" />
              <StoreButton img="https://upload.wikimedia.org/wikipedia/commons/3/3c/Download_on_the_App_Store_Badge.svg" text="Скоро в App Store" />
            </div>
          </motion.div>

          {/* --- 2. Интерфейс смартфона --- */}
          <motion.div 
                    ...
                    <AnimatePresence mode="wait">
                      {appState === 'loading' && (
                        <motion.div key="l" exit={{ opacity: 0 }} className="absolute inset-0 flex flex-col items-center justify-center bg-black">
                           ...
                           <span className="text-amber-500 font-bold text-[10px] tracking-[0.5em]">ШИФРОВАНИЕ</span>
                        </motion.div>
                      )}
                      ...
                      {appState === 'dashboard' && (
                        <motion.div key="d" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 bg-[#080808] flex flex-col pt-12 text-left">
                          ...
                          <div className="px-6 mb-8">
                            <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest mb-1">Общий баланс</p>
                          ...
                          <div className="mt-auto bg-[#111] rounded-t-[3rem] p-7 border-t border-white/10">
                            <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-6">Последние действия</h4>
                            <div className="space-y-5">
                              <TransactionItem type="up" title="Платеж получен" amount="+ 15k AFN" />
                              <TransactionItem type="down" title="Комиссия за обмен" amount="- $12.00" />
                            </div>
                          </div>
                        </motion.div>
                      )}
        ...
        {/* --- 3. Раздел преимуществ --- */}
        <div className="mt-32 md:mt-48 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 text-left">
          <FeatureCard 
            icon={<Zap size={28} />} 
            title="Молниеносно" 
            desc="Современная система обмена, минимизирующая время транзакций по всему миру." 
            delay={0.1} 
          />
          <FeatureCard 
            icon={<ShieldCheck size={28} />} 
            title="Cold Security" 
            desc="Максимальный уровень шифрования и холодное хранение для защиты ваших цифровых активов." 
            delay={0.2} 
          />
          <FeatureCard 
            icon={<Smartphone size={28} />} 
            title="Умный интерфейс" 
            desc="Разработан для упрощения управления даже самыми сложными финансовыми операциями." 
            delay={0.3} 
          />
          <FeatureCard 
            icon={<Clock size={28} />} 
            title="Поддержка 24/7" 
            desc="Наша техническая команда всегда на связи и готова помочь в любое время." 
            delay={0.4} 
          />
        </div>
        ...
        >
          <h2 className="text-3xl md:text-5xl font-black mb-12 italic uppercase tracking-tighter text-amber-500">С чего начать</h2>
          <div className="grid md:grid-cols-3 gap-12">
            <Step num="01" title="Скачать" desc="Запишитесь в лист ожидания, чтобы первыми испытать наше приложение." delay={0.1} />
            <Step num="02" title="Проверить" desc="Пройдите безопасную процедуру KYC для активации вашего глобального кошелька." delay={0.2} />
            <Step num="03" title="Торговать" desc="Обменивайте и управляйте своими активами без каких-либо границ." delay={0.3} />
          </div>
        </motion.div>
      </div>
    </div>
