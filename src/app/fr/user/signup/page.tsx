...
// Section de particules pour simuler des confettis (Simple Confetti)
...
export default function SignUpPage() {
  ...
  const [showSuccess, setShowSuccess] = useState(false); // État d'affichage du succès
  ...
  return (
    <div className="min-h-screen bg-[#000000] text-white flex flex-col md:flex-row overflow-hidden relative" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      {/* Animation de succès (Success Overlay) */}
      <AnimatePresence>
        {showSuccess && (
          <motion.div 
            initial={{ opacity: 0 }}
            ...
            >
              ...
              <div className="bg-zinc-900 border border-green-500/30 p-12 rounded-[4rem] flex flex-col items-center gap-6 shadow-[0_0_50px_rgba(34,197,94,0.2)]">
                ...
                <div className="text-center">
                  <h2 className="text-2xl font-black uppercase tracking-tighter">Bienvenue chez SafiPay</h2>
                  <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mt-2">Sécurisation de votre avenir financier...</p>
                </div>
              </div>
            </motion.div>
          </motion.div>
          ...
          <div className="flex flex-col lg:flex-row gap-12 items-start relative z-10">
            <div className="flex-1 space-y-6">
              <div className="flex items-center gap-2 text-amber-500">
                <Sparkles size={20} />
                <span className="text-[10px] font-black uppercase tracking-[0.3em]">Rejoignez l’avenir</span>
              </div>
              <h1 className="text-4xl font-black tracking-tighter leading-none">CRÉER <br/> <span className="text-amber-500">UN COMPTE</span></h1>
              <p className="text-zinc-500 text-sm leading-relaxed max-w-xs">
                {isRtl 
                  ? "Rejoignez le premier système bancaire connecté pour l’Afghanistan et le monde. Remplissez tous les champs avec soin et conformément à vos documents légaux."
                  : "Rejoignez le premier système bancaire connecté pour l’Afghanistan et le monde. Remplissez soigneusement tous les champs."
                }
              </p>
            </div>
            ...
            <div className="flex-[1.2] w-full">
              <form onSubmit={handleSignUp} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="relative group">
                  ...
                  <input 
                    type="text" required placeholder={isRtl ? "Prénom" : "First Name"}
                ...
                <div className="relative group">
                  <User className={`absolute top-1/2 -translate-y-1/2 ${isRtl ? 'left-5' : 'right-5'} text-zinc-600 group-focus-within:text-amber-500 w-4 h-4`} />
                  <input 
                    type="text" required placeholder={isRtl ? "Nom de famille" : "Last Name"}
                ...
                <div className="md:col-span-2 relative group">
                  ...
                  <input 
                    type="email" required placeholder="Adresse e-mail"
                ...
                >
                  <span className="uppercase tracking-widest text-xs">
                    {loading ? <Loader2 className="animate-spin" /> : 'S’inscrire en toute sécurité'}
                  </span>
              ...
              <div className="text-center pt-6">
                <Link href={`/${lang}/user/login`} className="text-zinc-600 hover:text-white text-[10px] font-bold tracking-widest uppercase transition-all">
                  Déjà membre ? <span className="text-amber-500">Connexion sécurisée</span>
                </Link>
              </div>
            </div>
          </div>
