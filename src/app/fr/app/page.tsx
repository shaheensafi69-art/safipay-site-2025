      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        <div className="flex flex-col md:grid md:grid-cols-2 gap-12 md:gap-16 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/5 border border-white/10 rounded-full text-amber-500 text-[10px] md:text-xs font-black tracking-widest mb-6 uppercase">
              ...
              Écosystème SafiPay en direct
            </div>
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-black mb-6 tracking-tighter italic leading-[0.9]">
              L'avenir des <span className="text-amber-500 text-glow">Finances</span> <br className="hidden md:block" /> 
              Dans votre poche
            </h1>
            <p className="text-gray-400 text-lg md:text-xl mb-10 leading-relaxed max-w-xl mx-auto md:mx-0 font-medium">
              Nous ne construisons pas seulement une application ; nous ouvrons une porte vers la liberté financière. Rapide, sécurisé et sans frontières.
            </p>
            <div className="flex flex-wrap justify-center md:justify-start gap-5">
              <StoreButton img="https://upload.wikimedia.org/wikipedia/commons/7/78/Google_Play_Store_badge_EN.svg" text="Bientôt sur Play Store" />
              <StoreButton img="https://upload.wikimedia.org/wikipedia/commons/3/3c/Download_on_the_App_Store_Badge.svg" text="Bientôt sur App Store" />
            </div>
          </motion.div>
                    ...
                    <AnimatePresence mode="wait">
                      {appState === 'loading' && (
                        <motion.div key="l" exit={{ opacity: 0 }} className="absolute inset-0 flex flex-col items-center justify-center bg-black">
                           ...
                           <span className="text-amber-500 font-bold text-[10px] tracking-[0.5em]">CHIFFREMENT</span>
                        </motion.div>
                      )}
                      ...
                      {appState === 'dashboard' && (
                        <motion.div key="d" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 bg-[#080808] flex flex-col pt-12 text-left">
                          ...
                          <div className="px-6 mb-8">
                            <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest mb-1">Solde Total</p>
                          ...
                          <div className="mt-auto bg-[#111] rounded-t-[3rem] p-7 border-t border-white/10">
                            <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-6">Activités Récentes</h4>
                            <div className="space-y-5">
                              <TransactionItem type="up" title="Paiement Reçu" amount="+ 15k AFN" />
                              <TransactionItem type="down" title="Frais de Change" amount="- $12.00" />
                            </div>
                          </div>
                        </motion.div>
                      )}
        ...
        <div className="mt-32 md:mt-48 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 text-left">
          <FeatureCard 
            ...
            desc="Système d'échange moderne conçu pour minimiser les temps de transaction dans le monde entier." 
            delay={0.1} 
          />
          <FeatureCard 
            icon={<ShieldCheck size={28} />} 
            title="Sécurité Froide" 
            desc="Niveau de chiffrement maximal et stockage à froid pour protéger vos actifs numériques." 
            delay={0.2} 
          />
          <FeatureCard 
            icon={<Smartphone size={28} />} 
            title="Interface Intelligente" 
            desc="Conçue pour simplifier la gestion des transactions financières les plus complexes." 
            delay={0.3} 
          />
          <FeatureCard 
            icon={<Clock size={28} />} 
            title="Support 24/7" 
            desc="Notre équipe technique est à votre disposition à tout moment pour vous aider." 
            delay={0.4} 
          />
        </div>
        ...
        >
          <h2 className="text-3xl md:text-5xl font-black mb-12 italic uppercase tracking-tighter text-amber-500">Premiers Pas</h2>
          <div className="grid md:grid-cols-3 gap-12">
            <Step num="01" title="Télécharger" desc="Inscrivez-vous sur la liste d'attente pour être parmi les premiers à découvrir l'application." delay={0.1} />
            <Step num="02" title="Vérifier" desc="Complétez le processus KYC sécurisé pour activer votre portefeuille global." delay={0.2} />
            <Step num="03" title="Échanger" desc="Échangez et gérez vos actifs sans aucune frontière." delay={0.3} />
          </div>
        </motion.div>
      </div>
    </div>
