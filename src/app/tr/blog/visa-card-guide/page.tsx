...
export default function VisaCardGuidePage() {
  ...
  // Dil yapılandırması parametrelere göre veya varsayılan olarak Türkçe
  const lang = params?.lang || 'tr';
  const isRtl = lang === 'ar' || lang === 'fa' || lang === 'ps';

  const content = {
    tr: {
      category: "Stratejik Analiz ve Rehber",
      title: "SafiPay Sanal Visa Kartı;",
      subtitle: "Afganistan'daki Finansal İzolasyona Son",
      author: "Safi AI",
      role: "SafiPay Resmi Sözcüsü",
      readTime: "15 dk okuma",
      intro: "Afganistan'daki birçok yerel banka, yalnızca ulusal sınırlar içinde geçerli olan kartlar basmaktadır. SafiPay bu engelleri yıktı. Avrupa'nın kalbinde, ABD'de ve tüm küresel web sitelerinde ve platformlarda güçlü bir şekilde çalışan kartlar sunuyoruz.",
      advantageTitle: "Visa Kartımız Neden Farklı?",
      advantageText: "Afganistan'daki mevcut banka kartlarının en büyük sorunu, küresel ödeme sistemleriyle olan bağlantı eksikliğidir. SafiPay kartları doğrudan Avrupa Birliği finans kuruluşları tarafından ihraç edilmektedir. Bu, Kabil'den, Herat'tan veya dünyanın herhangi bir yerinden harcamalarınızı işlem reddi veya bloke edilme korkusu olmadan yapabileceğiniz anlamına gelir.",
      comparisonTitle: "Bir Bakışta Temel Farklar",
      comp1: "Küresel İşlevsellik: Yerel bankaların aksine, kartımız coğrafi kısıtlama olmaksızın tüm uluslararası ödeme altyapılarında %100 aktiftir.",
      comp2: "Düzenleme Hızı: Geleneksel bankalar sizi basit bir kart için haftalarca bekletirken, SafiPay'de dijital kart ihracı 1 dakikadan kısa sürer.",
      comp3: "Teknik Güvenlik: Müjtəba (Teknik Güvenlik Müdürü) tarafından doğrudan denetlenen tüm protokollerde üst düzey şifreleme garantisi.",
      techTitle: "Teknik ve Yönetimsel Altyapı",
      techDesc: "Operasyon ve Teknik Güvenlik Müdürümüz Müjtəba, işlemlerinizin katı uluslararası filtreleri aşmasını sağlayan gelişmiş protokoller uyguladı. Ayrıca, Sahel (Uluslararası Geliştirme Müdürü), bu kartların sorunsuz entegrasyon için Avrupa para yasaları ve IBAN hesaplarıyla tam uyumlu olmasını sağladı.",
      quote: "Sadece bir kart çıkarmıyoruz; her Afgan vatandaşı için küresel ekonominin kapılarını açıyoruz. Bağlantıda olmak sizin hakkınızdır.",
      quoteAuthor: "Shahin Safi — SafiPay Kurucusu",
      backBtn: "SafiPay Ansiklopedisine Dön"
    }
  };
            ...
            <article className={`space-y-16 text-gray-300 leading-[2.4] ${isRtl ? 'text-right' : 'text-left'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10 my-24">
                <div className="p-12 rounded-[3rem] bg-white/[0.02] border border-white/10 transition-all duration-500 hover:scale-105 hover:bg-white/[0.05] group">
                  ...
                  <h4 className="text-white font-black text-2xl mb-4">Küresel Alışveriş</h4>
                  <p className="text-gray-500 leading-relaxed text-sm">SafiPay'in seçkin güvenliği ile Amazon, eBay ve tüm uluslararası ödeme kanallarına sınırsız erişim sağlayın.</p>
                </div>
                <div className="p-12 rounded-[3rem] bg-white/[0.02] border border-white/10 transition-all duration-500 hover:scale-105 hover:bg-white/[0.05] group">
                  ...
                  <h4 className="text-white font-black text-2xl mb-4">Avrupa Güvenliği</h4>
                  <p className="text-gray-500 leading-relaxed text-sm">İşlemleriniz doğrudan Müjtəba tarafından izlenir ve Avrupa Birliği bankacılık standartlarıyla tam uyumludur.</p>
                </div>
              </div>
            </article>
