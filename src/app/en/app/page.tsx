...
export default function AppDownloadPage() {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#050505] text-white pt-20 md:pt-32 pb-10 px-4 md:px-6 relative overflow-hidden">
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        <div className="flex flex-col md:grid md:grid-cols-2 gap-12 md:gap-16 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/5 border border-white/10 rounded-full text-amber-500 text-[10px] md:text-xs font-black tracking-widest mb-6 uppercase">
              ...
              SafiPay Ecosystem Live
            </div>
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-black mb-6 tracking-tighter italic leading-[0.9]">
              Future <span className="text-amber-500 text-glow">Finance</span> <br className="hidden md:block" /> 
              In Your Pocket
            </h1>
            <p className="text-gray-400 text-lg md:text-xl mb-10 leading-relaxed max-w-xl mx-auto md:mx-0 font-medium">
              We aren't just building an app; we're opening a gateway to financial freedom. Fast, secure, and borderless.
            </p>
            <div className="flex flex-wrap justify-center md:justify-start gap-5">
              <StoreButton img="https://upload.wikimedia.org/wikipedia/commons/7/78/Google_Play_Store_badge_EN.svg" />
              <StoreButton img="https://upload.wikimedia.org/wikipedia/commons/3/3c/Download_on_the_App_Store_Badge.svg" />
            </div>
          </motion.div>

          {/* --- ۲. گوشی هوشمند (بدون حذفیات) --- */}
          <motion.div 
            style={{ y: yPhone, rotateZ: rotatePhone }} 
            className="relative flex justify-center order-1 md:order-2 w-full"
          >
                    ...
                    <AnimatePresence mode="wait">
                      {appState === 'loading' && (
                        <motion.div key="l" exit={{ opacity: 0 }} className="absolute inset-0 flex flex-col items-center justify-center bg-black">
                           ...
                           <span className="text-amber-500 font-bold text-[10px] tracking-[0.5em]">ENCRYPTING</span>
                        </motion.div>
                      )}

                      {appState === 'dashboard' && (
                        <motion.div key="d" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 bg-[#080808] flex flex-col pt-12 text-left" dir="ltr">
                          ...
                          <div className="px-6 mb-8">
                            <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest mb-1">Total Balance</p>
                          ...
                          <div className="mt-auto bg-[#111] rounded-t-[3rem] p-7 border-t border-white/10">
                            <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-6">Recent Activity</h4>
                            <div className="space-y-5">
                              <TransactionItem type="up" title="Transfer Received" amount="+ 15k AFN" />
                              <TransactionItem type="down" title="Exchange Fee" amount="- $12.00" />
                            </div>
                          </div>
                        </motion.div>
                      )}
        ...
        {/* --- ۳. بخش ویژگی‌ها (کامل با توضیحات) --- */}
        <div className="mt-32 md:mt-48 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6" dir="ltr">
          <FeatureCard 
            icon={<Zap size={28} />} 
            title="Lightning Fast" 
            desc="Modern exchange system designed to minimize transaction time globally." 
            delay={0.1} 
          />
          <FeatureCard 
            icon={<ShieldCheck size={28} />} 
            title="Cold Security" 
            desc="Highest level of encryption and cold storage to protect your digital assets." 
            delay={0.2} 
          />
          <FeatureCard 
            icon={<Smartphone size={28} />} 
            title="Smart Interface" 
            desc="Engineered for simplicity in managing the most complex financial trades." 
            delay={0.3} 
          />
          <FeatureCard 
            icon={<Clock size={28} />} 
            title="24/7 Readiness" 
            desc="Our technical support team is always active whenever you need assistance." 
            delay={0.4} 
          />
        </div>
        ...
        <motion.div 
          ...
          dir="ltr"
        >
          <h2 className="text-3xl md:text-5xl font-black mb-12 italic uppercase tracking-tighter">Get Started</h2>
          <div className="grid md:grid-cols-3 gap-12">
            <Step num="01" title="Download" desc="Join our waiting list to be among the first to experience the app." delay={0.1} />
            <Step num="02" title="Verify" desc="Complete the secure KYC process to activate your global wallet." delay={0.2} />
            <Step num="03" title="Trade" desc="Start exchanging and managing your assets with zero borders." delay={0.3} />
          </div>
        </motion.div>
      </div>
    </div>
...
function CurrencyCard({ flag, code, amount, highlight }: any) {
  return (
    <div className={`min-w-[125px] p-5 rounded-[2rem] border ${highlight ? 'bg-amber-500 border-amber-600' : 'bg-white/5 border-white/10'}`}>
...
function StoreButton({ img }: any) {
  return (
    <motion.div whileHover={{ scale: 1.05 }} className="px-6 py-3.5 bg-white/5 border border-white/10 rounded-2xl grayscale opacity-60">
