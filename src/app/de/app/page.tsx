      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        <div className="flex flex-col md:grid md:grid-cols-2 gap-12 md:gap-16 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/5 border border-white/10 rounded-full text-amber-500 text-[10px] md:text-xs font-black tracking-widest mb-6 uppercase">
              ...
              SafiPay Ökosystem Live
            </div>
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-black mb-6 tracking-tighter italic leading-[0.9]">
              Zukunft der <span className="text-amber-500 text-glow">Finanzen</span> <br className="hidden md:block" /> 
              In Ihrer Tasche
            </h1>
            <p className="text-gray-400 text-lg md:text-xl mb-10 leading-relaxed max-w-xl mx-auto md:mx-0 font-medium">
              Wir bauen nicht nur eine App; wir eröffnen ein Tor zur finanziellen Freiheit. Schnell, sicher und grenzenlos.
            </p>
            <div className="flex flex-wrap justify-center md:justify-start gap-5">
              <StoreButton img="https://upload.wikimedia.org/wikipedia/commons/7/78/Google_Play_Store_badge_EN.svg" text="Bald im Play Store" />
              <StoreButton img="https://upload.wikimedia.org/wikipedia/commons/3/3c/Download_on_the_App_Store_Badge.svg" text="Bald im App Store" />
            </div>
          </motion.div>
                    ...
                    <AnimatePresence mode="wait">
                      {appState === 'loading' && (
                        <motion.div key="l" exit={{ opacity: 0 }} className="absolute inset-0 flex flex-col items-center justify-center bg-black">
                           ...
                           <span className="text-amber-500 font-bold text-[10px] tracking-[0.5em]">VERSCHLÜSSELUNG</span>
                        </motion.div>
                      )}
                      ...
                      {appState === 'dashboard' && (
                        <motion.div key="d" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 bg-[#080808] flex flex-col pt-12 text-left">
                          ...
                          <div className="px-6 mb-8">
                            <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest mb-1">Gesamtguthaben</p>
                          ...
                          <div className="mt-auto bg-[#111] rounded-t-[3rem] p-7 border-t border-white/10">
                            <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-6">Letzte Aktivitäten</h4>
                            <div className="space-y-5">
                              <TransactionItem type="up" title="Zahlung Erhalten" amount="+ 15k AFN" />
                              <TransactionItem type="down" title="Wechselgebühr" amount="- $12.00" />
                            </div>
                          </div>
                        </motion.div>
                      )}
        ...
        <div className="mt-32 md:mt-48 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 text-left">
          <FeatureCard 
            icon={<Zap size={28} />} 
            title="Blitzschnell" 
            desc="Modernes Exchange-System zur Minimierung der Transaktionszeiten weltweit." 
            delay={0.1} 
          />
          <FeatureCard 
            icon={<ShieldCheck size={28} />} 
            title="Cold Security" 
            desc="Höchste Verschlüsselungsstufe und Cold Storage zum Schutz Ihrer digitalen Assets." 
            delay={0.2} 
          />
          <FeatureCard 
            icon={<Smartphone size={28} />} 
            title="Smart Interface" 
            desc="Entwickelt für Einfachheit bei der Verwaltung komplexester Finanzgeschäfte." 
            delay={0.3} 
          />
          <FeatureCard 
            icon={<Clock size={28} />} 
            title="24/7 Support" 
            desc="Unser technisches Team ist jederzeit für Sie da, wenn Sie Hilfe benötigen." 
            delay={0.4} 
          />
        </div>
        ...
        >
          <h2 className="text-3xl md:text-5xl font-black mb-12 italic uppercase tracking-tighter text-amber-500">Erste Schritte</h2>
          <div className="grid md:grid-cols-3 gap-12">
            <Step num="01" title="Download" desc="Tragen Sie sich in die Warteliste ein, um die App als Erster zu erleben." delay={0.1} />
            <Step num="02" title="Verifizieren" desc="Schließen Sie den KYC-Prozess ab, um Ihr globales Wallet zu aktivieren." delay={0.2} />
            <Step num="03" title="Handeln" desc="Tauschen und verwalten Sie Ihre Vermögenswerte ohne Grenzen." delay={0.3} />
          </div>
        </motion.div>
      </div>
    </div>
