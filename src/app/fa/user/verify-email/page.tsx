...
import { Mail, ShieldCheck, ExternalLink, Sparkles, MoveLeft, Loader2, CheckCircle2 } from 'lucide-react';
import { useParams, useRouter } from 'next/navigation';
// استفاده از متد جدید برای کلاینت
...
export default function VerifyEmailLandscape() {
  ...
  // ایجاد کلاینت سوپابیس به روش جدید
  ...
  const handleResend = async () => {
    ...
    // گرفتن اطلاعات کاربر فعلی که در حال تایید است
    ...
    if (user?.email) {
      ...
      // اگر سشن پریده بود، کاربر را به لاگین بفرست
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#020202] text-white flex items-center justify-center px-6 relative overflow-hidden font-sans">
      
      {/* پس‌زمینه کیهانی */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-20%] right-[-10%] w-[800px] h-[800px] bg-amber-500/5 blur-[120px] rounded-full" />
        <div className="absolute bottom-[-20%] left-[-10%] w-[600px] h-[600px] bg-blue-600/5 blur-[150px] rounded-full" />
      </div>
      ...
      >
        <div className="flex flex-col md:flex-row min-h-[500px]">
          ...
          {/* بخش چپ: برندینگ */}
          <div className="w-full md:w-[40%] bg-gradient-to-b from-zinc-800/40 to-black p-12 flex flex-col justify-between border-l border-white/5">
             ...
             <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.3em] text-zinc-600">
                <ShieldCheck size={14} className="text-green-600" /> رمزنگاری سرتاسری
             </div>
          </div>

          {/* بخش راست: تعامل کاربر */}
          <div className="w-full md:w-[60%] p-12 md:p-20 flex flex-col justify-center bg-black/40 text-right">
            <h1 className="text-5xl md:text-7xl font-black italic uppercase tracking-tighter mb-6 leading-[0.85]">
              تایید <br/><span className="text-amber-500 text-6xl md:text-8xl">هویت</span>
            </h1>
            <p className="text-zinc-500 text-lg mb-10 max-w-sm leading-relaxed">
              ما یک لینک امن به صندوق ایمیل شما ارسال کرده‌ایم. برای آغاز بانکداری بین‌المللی، <span className="text-white">وضعیت ویژه</span> خود را فعال کنید.
            </p>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              >
                باز کردن ایمیل <ExternalLink size={14} />
              </a>
              
              <button 
                onClick={() => router.push(`/${locale}/user/signup`)}
              ...
              >
                بازگشت به ثبت‌نام <MoveLeft size={14} />
              </button>
            </div>
              ...
              <AnimatePresence mode="wait">
                {status === 'success' ? (
                  <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 text-green-500 text-[10px] font-black uppercase tracking-widest">
                    <CheckCircle2 size={14} /> لینک جدید با موفقیت ارسال شد
                  </motion.div>
                ) : status === 'error' ? (
                  <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="text-red-500 text-[10px] font-black uppercase tracking-widest">
                    ارسال دوباره موفق نشد. بعداً دوباره تلاش کنید
                  </motion.div>
                ) : (
                  <button 
                  ...
                  >
                    ...
                    ایمیل را دریافت نکردید؟ <span className="text-amber-500 group-hover:underline">ارسال دوباره لینک</span>
                  </button>
                )}
              </AnimatePresence>
            </div>
